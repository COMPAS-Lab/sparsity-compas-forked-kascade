...
class KascadeStrategy(PostSoftmaxPooledPrefillTopkStrategy): # Does not support stats collection since never used for stats collection
    def __init__(self, recompute_layers: List[int], model_name: str, name="kascade", k=1, tile_size=1, rolling_prefill=False, block_size=12288):
        ...
        self._head_mapping = self.get_head_mapping(model_name)
        ...
        **kwargs,
        ...
        if attention_mask is not None:
            ...
            if not (module.layer_idx in self.recompute_layers
                ...
                indices = self._topk_to_reuse[:, self._head_mapping[module.layer_idx], start:end, :].to(torch.int64)
        ...
        if (module.layer_idx in self.recompute_layers
            ...
            if self.tile_size > 1 and Lq > self.tile_size:
                ...
                attn_weights_per_khead = padded.view(B, H//module.num_key_value_groups, num_tiles, self.tile_size, Lk).sum(dim=-2) # [B, H//4, Lq // tile_size, Lk]
                attn_weights_per_khead.multiply_((exclude_last_tile_mask==0).to(attn_weights_per_khead.dtype))  # [B, H//4, Lq // tile_size, Lk]
                ...
                indices = indices.repeat_interleave(self.tile_size, dim=-2)   # [B, H//4, Lq_with_padding, Lk]
                indices = indices[:, :, :Lq, :]  # [B, H, Lq, Lk]
            else:
                indices = torch.topk(attn_weights_per_khead, k, dim=-1).indices # [B, H//4, Lq, Lk]
    ...
    def attention_forward(
        ...
        **kwargs,
        ...
        if module.layer_idx == 0:
            self._topk_to_reuse = torch.zeros((B, G, Lq, k), device=query.device, dtype=torch.int32)
    ...
    def get_head_mapping(self, model_name: str):
        try:
            import numpy as np
            model_basename = model_name.split('/')[-1]
            head_mapping = np.load(
                f"./results/head_mappings/{model_basename}_layers_{"_".join(map(str, self.recompute_layers))}.npy"
            ).tolist()
        except Exception:
